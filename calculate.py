...
class Calculator(QMainWindow):  # 定义Calculator类，继承自QMainWindow
    ...
    def initUI(self):
        ...
        for button_text in buttons:
            ...
            button.setFixedSize(85, 65)  # 设置按钮大小为
            grid_layout.addWidget(button, row, col)  # 将按钮添加到网格布局中

            col += 1
            if col > 3:  # 每行4个按钮，超过则转到下一行
                col = 0
                row += 1

            # 如果按钮文本是 "="，则设置背景色为蓝色
            if button_text == '=':
                button.setStyleSheet("background-color: blue; color: white;")  # 设置蓝色背景和白色文字
        ...
        if button:
            ...
            if text == 'CE':
                ...
                try:
                    ...
                    if denominator == 0:
                        raise ZeroDivisionError("0没有倒数")  # 自定义除数为0错误
